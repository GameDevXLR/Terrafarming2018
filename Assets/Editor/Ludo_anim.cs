...
public class Ludo_anim : MonoBehaviour {
    ...
    void Update () {
        ...
        //TOURNER A GAUCHE
        if (rotation < -0.5 || Input.GetKey(KeyCode.LeftArrow)) //Input.GetKey(KeyCode.LeftArrow)
        {
            transform.Rotate(0, -(anim.speed * Time.deltaTime)*70, 0 );
        }

        //TOURNER A DROITE
        if ((rotation > 0.5) || Input.GetKey(KeyCode.RightArrow)) //Input.GetKey(KeyCode.RightArrow)
        {
            transform.Rotate(0, (anim.speed * Time.deltaTime) * 70, 0);
        }
